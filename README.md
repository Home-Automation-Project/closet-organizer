...
# Overview
Closet organization with three types of organization.  One is for bedding.  There will be a shelf pressure mat or load cell → tracks presence, not exact weight.  If shelf goes from “occupied” → “empty” then Log usage and add reminder: “Wash sheets within 24h.”  Second group is for cleaning supplies.  There will be a cleaning bin for each cleaning type (bathroom, bedroom, kitchen, library, etc).  NFC on bin → tap → shows: Last cleaned date; Recommended frequency; Checklist for that room.  Plus there should be seasonal reminders such as : “Replace sponges”, “Refill floor cleaner”, “Check spray bottle nozzles.”  These notices can show up on the web UI (explained later) and be sent as a MQTT alert to the home automation server.  The third group is first aid.

There is an API (fastapi) and database (postgres) backend.  The frontend UI (vue.js) will show each closet shelf and a basket representation for each basket in the closet.  If the basket has items missing it will appear yellow.  If expired (if applicable) it will appear red.  The basket will have a title display showing what it is (e.g. wound, bedroom, etc).    Clicking on the basket icon will show the inventory of that basket, quantity and expiration date (if applicable).  One can modify inventory (add/subtract/edit) on this page too.  There is a separate "setup page" which allows the end-user to specify number of shelves, which baskets are on which shelf, in which order, etc.  There can be multiple "closets" in a home.  Each basket will have either a QR code or NFC tag which should open a page show this bin/basket inventory page with what should be in the bin, quantity (low/ok/overstocked flags) and expiration dates (if applicable, e.g. medication).  Last there should be a first-aid section of the website which will have sub-pages of CPR steps; poison control number; family allergies/med lists. Lastly a cleaning checklist section.  There will be a cleaning checklist page for each room: (library, kitchen, dinning room, gym, bedroom, bathroom, playroom, hallways, laundry room, garage, family room).  This will be accessible from people's phone by scanning a QR code for that room type and they should be able to click the items as they finish them.  The list should include the categories, and items, of things which should be cleaned seasonally, weekly, monthly and daily.  All the other pages mentioned will probably be viewed from a tablet which will be placed on the outside of the closet's doors.

Project is fully containerized.
---
